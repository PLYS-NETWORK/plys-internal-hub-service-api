...
> **Cache invalidation:** every mutating endpoint here calls `BoardCacheService.invalidateProject` so the BUSINESS-side cached board (see [business board-api-specs.md](../../business/projects/board-api-specs.md)) reflects the change on the next list call.
