...
While the consultant's account has `users.is_active = false` (CopyLeaks 3-strike ban), every endpoint above is unreachable — every active session was revoked at ban time, so even cached JWTs immediately fail with `403 AUTH_ACCOUNT_INACTIVE`. See [auth/consultant.md](../auth/consultant.md) for the full ban/block matrix.
