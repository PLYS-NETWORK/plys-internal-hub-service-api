...
- **Side effects:**
  ...
  | REJECTED | `onboarding.status = REJECTED`, `reviewed_by/reviewed_at`, `rejection_note`, `blocked_until = now + 3 months`. | Email: `sendApplicationRejectedEmail` with `reason` + `blocked_until`. Auth flow then refuses login / register / profile-submit for 3 months — see [auth/consultant.md](../auth/consultant.md#onboarding-rejection-block-read-first). |
