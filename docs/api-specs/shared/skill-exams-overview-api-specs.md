...
Every terminal skill-exam transition (PASSED / FAILED / EXPIRED / COPYLEAKS_FAILED) and every CopyLeaks-ban fans out an in-app notification to every active admin. See the [admin notification catalog](../admin/notifications/notifications-admin-events-api-specs.md) for the `admin_skill_exam_result` and `admin_consultant_banned` payloads.

## Companion docs

- [Auth — Consultant](../consultant/auth/auth-api-specs.md) — banned-account login behaviour.
- Notifications — [consultant catalog](../consultant/notifications/notifications-consultant-events-api-specs.md) · [admin catalog](../admin/notifications/notifications-admin-events-api-specs.md).
