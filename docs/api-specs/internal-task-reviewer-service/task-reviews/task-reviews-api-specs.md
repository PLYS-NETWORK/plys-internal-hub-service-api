...
> **Source:** [apps/internal-task-reviewer-service/src/modules/task-reviews/task-reviews.controller.ts](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/task-reviews.controller.ts)
> **Base path:** `/api/v1/admin/task-reviews`
...
- `REVISION_REQUESTED` — round has resolved with a bounce-back. Consultant resubmits via the [consultant tasks endpoint](../../consultant-service/projects/tasks-api-specs.md); a new round begins (`last_review_round` increments).
...
All endpoints use [`THROTTLE_DEFAULT`](../../../../apps/internal-task-reviewer-service/src/common/constants/throttle.constants.ts).
...
- **Method:** `GET`
- **Query params:** [`ListPendingReviewsDto`](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/dto/requests/list-pending-reviews.dto.ts) extends [`PageOptionsDto`](../../../../packages/common-nest/dto/page-options.dto.ts)
...
- **Response 200:** [`ITaskReviewDetailResponse`](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/dto/responses/interfaces/task-review.response.interface.ts)
...
- **Request body:** [`SubmitVoteDto`](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/dto/requests/submit-vote.dto.ts)
...
- **Behaviour:**
  ...
  5. Resolution branch (the side-effects below run **after** the vote tx commits, so the reviewer's response returns first):
     ...
     | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
     | 2 votes, both PASS (no arbiter yet)                     | Task `kanban_status = 'pending_approval'`. Emit `TASK_AI_REVIEW_REQUESTED` event → [`TaskAiReviewHandler`](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/handlers/task-ai-review.handler.ts) runs the stub AI gate (`AiQualityCheckService` currently returns `pass`) and calls `markDone` or `markRevisionRequested`. |
...
1. A platform admin invites the reviewer's email via `POST /admin/allowed-emails` with `{ "role": "TASK_REVIEWER" }`. See the [allowed-emails spec](../identity-service/auth/admin-allowed-emails-api-specs.md).
...
3. A consultant submits a task for review. `ConsultantProjectTasksService.submitForReview` increments `tasks.last_review_round` and calls `TaskReviewAssignmentService.assignInitialReviewers`, which picks **2 eligible** reviewers via [`TaskRepository.pickEligibleReviewers`](../../../../packages/unit-of-work/repositories/tasks/task.repository.ts) — `role = 'TASK_REVIEWER' AND is_active = TRUE`, excluding the consultant + project members, round-robin biased toward the least recently assigned.
...
- **Service entry-points:** [`TaskReviewVotingService`](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/services/task-review-voting.service.ts), [`TaskReviewAssignmentService`](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/services/task-review-assignment.service.ts), [`TaskCompletionService`](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/services/task-completion.service.ts), [`AiQualityCheckService`](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/services/ai-quality-check.service.ts) (current implementation is a stub returning PASS).
- **Async handler:** [`TaskAiReviewHandler`](../../../../apps/internal-task-reviewer-service/src/modules/task-reviews/handlers/task-ai-review.handler.ts) — `@OnEvent(TASK_AI_REVIEW_REQUESTED)`. Idempotent: re-checks task status before running.
- **Notification types:**
  ...
  - [`consultant_task_status_changed`](../../consultant-service/notifications/notifications-consultant-events-api-specs.md) — sent to the consultant on `done` (with `earned_amount`), `revision_requested` (with `feedback_summary`, `revision_count`, `revisions_remaining`), or `pending_approval` on cap-escalation.
- **Repository:** [`TaskReviewRepository`](../../../../packages/unit-of-work/repositories/tasks/task-review.repository.ts) — owns the pessimistic-write lock, the unique constraint check, and the tally aggregation.
- **Consultant submit-for-review:** [tasks-api-specs.md § 4](../../consultant-service/projects/tasks-api-specs.md#4-submit-task-for-review) — the trigger that enrols the task into this controller's flow.
