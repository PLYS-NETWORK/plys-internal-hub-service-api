...
> The `CONSULTANT_ONBOARDING_BLOCKED` (3-month admin-rejection block) does **not** reach these endpoints — it is enforced at the auth layer, so the consultant cannot sign in to obtain a token in the first place. See [auth-api-specs.md](../auth/auth-api-specs.md).
