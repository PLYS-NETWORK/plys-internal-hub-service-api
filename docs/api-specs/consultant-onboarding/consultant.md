...
While the consultant's latest onboarding has `blocked_until > now` (after an admin REJECT), **every** call above is unreachable: login already fails with `403 CONSULTANT_ONBOARDING_BLOCKED`, so the client never gets a session to call these endpoints. See [auth/consultant.md](../auth/consultant.md#onboarding-rejection-block-read-first) for the full block matrix.
