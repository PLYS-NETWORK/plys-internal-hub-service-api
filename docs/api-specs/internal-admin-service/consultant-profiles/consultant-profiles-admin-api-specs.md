...
> **Source:** [apps/api-gateway/src/http/v1/consultant/controllers/consultant-profiles-admin.controller.ts](../../../../apps/api-gateway/src/http/v1/consultant/controllers/consultant-profiles-admin.controller.ts)
> **Service DTOs:** [apps/consultant-service/src/modules/profiles/dto](../../../../apps/consultant-service/src/modules/profiles/dto)
> **Base path:** `/api/v1/admin/consultant-profiles`
...
- **Method:** `GET`
- **Query params:** [`ListConsultantProfilesAdminDto`](../../../../apps/internal-admin-service/src/modules/profiles/consultant/dto/requests/list-consultant-profiles-admin.dto.ts) (extends [`PageOptionsDto`](../../../../packages/common-nest/dto/page-options.dto.ts))
...
- **Behaviour:**
  ...
  - Re-presigns `avatar_url` for every row via [`UrlResolverService.resolveMany`](../../../../packages/common-nest/modules/file-storage/services/url-resolver.service.ts) so clients receive a freshly-signed URL (the stored value is the upload-time presign with a short TTL).
  - Pagination metadata is built via [`PageMetaDto`](../../../../packages/common-nest/dto/page-meta.dto.ts).
- **Response 200:** `PageDto<`[`IAdminConsultantProfileListItemResponse`](../../../../apps/internal-admin-service/src/modules/profiles/consultant/dto/responses/interfaces/admin-consultant-profile-list-item.response.interface.ts)`>`
...
- **Behaviour:**
  ...
  - Loads the consultant's skills via [`ConsultantSkillsService.findByConsultantId`](../../../../apps/internal-admin-service/src/modules/profiles/consultant/consultant-skills.service.ts) (one extra query — acceptable for an admin detail call).
  - Re-presigns `avatar_url` and `cv_url` in parallel via `UrlResolverService.resolve`.
- **Response 200:** [`IAdminConsultantProfileDetailResponse`](../../../../apps/internal-admin-service/src/modules/profiles/consultant/dto/responses/interfaces/admin-consultant-profile-detail.response.interface.ts) — superset of the user-scoped [`IConsultantProfileResponse`](../../../../apps/internal-admin-service/src/modules/profiles/consultant/dto/responses/interfaces/consultant-profile.response.interface.ts) plus admin-only columns and the joined user fields.
...
- **Service:** [ConsultantProfilesAdminService](../../../../apps/internal-admin-service/src/modules/profiles/consultant/consultant-profiles-admin.service.ts) — owns the QueryBuilder for the join, the `SORT_COLUMN_MAP` whitelist, and the URL resolution.
- **Repository accessor:** `uow.consultantProfiles` from [UnitOfWorkService](../../../../packages/unit-of-work/unit-of-work.service.ts).
- **User-scoped surface (separate controller):** [`ConsultantProfilesController`](../../../../apps/internal-admin-service/src/modules/profiles/consultant/consultant-profiles.controller.ts) — the `/consultant-profiles/me` routes used by consultants (get/update own profile).
...
- **Admin controller convention precedent:** [`BusinessProfilesAdminController`](../../../../apps/internal-admin-service/src/modules/profiles/business/business-profiles-admin.controller.ts) — same shape (`@Controller('admin/...')`, class-level `@Roles(UserRole.ADMIN_PLATFORM)`, no `@Platform`). See [business-profiles-admin-api-specs.md](../business-profiles/business-profiles-admin-api-specs.md).
