...
This surface is distinct from the OTP flow at [`/admin/auth/*`](../auth/admin.md). It governs **who is allowed onto the hub**, not the login itself.
...
The OTP request/verify endpoints at [`/admin/auth/*`](../auth/admin.md) call `AdminAllowedEmailRepository.findActiveByEmail` (active rows only). The endpoints above govern that table:
...
- **OTP flow spec (sibling):** [`docs/api-specs/auth/admin.md`](../auth/admin.md).
