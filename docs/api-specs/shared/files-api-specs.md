...
> **Source:** [src/modules/files/files.controller.ts](../../../src/modules/files/files.controller.ts)
> **Base path:** `/files`
...
- **Storage adapter contract:** [`IStorageProvider`](../../../src/common/modules/file-storage/interfaces/storage.provider.interface.ts) — `put` / `getUrl` / `download` / `remove`. Concrete adapters: [LocalStorageProvider](../../../src/common/modules/file-storage/providers/local-storage.provider.ts), [S3StorageProvider](../../../src/common/modules/file-storage/providers/s3-storage.provider.ts).
- **Service:** [FilesService](../../../src/modules/files/files.service.ts) — owns ownership gating, quota enforcement, sha256 hashing, and the `loadOwnedOrThrow` no-leak rule.
- **Cleanup cron:** [FilesCleanupService](../../../src/modules/files/files-cleanup.service.ts) — weekly orphan sweep (`purpose IS NULL` and unreferenced) and daily soft-delete reclamation.
- **Attachment surfaces (consume `file_id`):**
  - [task-attachments-api-specs.md](../business/projects/task-attachments-api-specs.md) — business owner's brief/reference uploads.
  - Board task results — see [board-api-specs.md](../business/projects/board-api-specs.md) (business view) and [board-api-specs.md](../consultant/projects/board-api-specs.md) (consultant view). Attachments expose `file_id` only; clients call `GET /files/:file_id/download` for bytes.
