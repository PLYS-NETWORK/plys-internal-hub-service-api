...
import { BffEnvelopeCipher } from '@plys/libraries/common-nest/crypto/ai-keys/bff-envelope.cipher';
import { MasterKeyCipher } from '@plys/libraries/common-nest/crypto/ai-keys/master-key.cipher';
...
import { GENERIC_ERROR_CODES } from '@plys/libraries/shared-kernel';
  ...
  public async getActiveKeyEnvelope(assistantType: AiAssistantType): Promise<ApiKeyBffResponseDto> {
    ...
    if (!row) {
      ...
      throw new TranslatableException({
        ...
        errorCode: AI_PROVIDER_KEY_ERROR_CODES.AI_PROVIDER_KEY_NOT_CONFIGURED,
    ...
    const activated = await this.uow.withTransaction(async (tx) => {
      ...
      if (!target) {
        ...
        throw new TranslatableException({
          ...
          errorCode: AI_PROVIDER_KEY_ERROR_CODES.AI_PROVIDER_KEY_NOT_FOUND,
          status: HttpStatus.NOT_FOUND,
        });
      }
      ...
      if (target.isActive) {
        ...
        if (otherCount === 0) {
          ...
          throw new TranslatableException({
            ...
            errorCode: AI_PROVIDER_KEY_ERROR_CODES.AI_PROVIDER_KEY_ACTIVE_REQUIRES_REPLACEMENT,
  ...
  private requireUserId(): string {
    ...
    if (!userId) {
      ...
      throw new TranslatableException({
        ...
        errorCode: GENERIC_ERROR_CODES.GENERIC_UNAUTHORIZED,
