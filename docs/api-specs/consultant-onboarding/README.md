...
| ---------------------------------------------------------------- | --------------------- | -------------------------------------------------------------------------------------- |
| [consultant.md](./consultant.md)                                 | Consultant (Lona app) | Basic profile (Step 1) + active-question fetch + bulk answer submission (Step 2)       |
| [admin.md](./admin.md)                                           | Internal Hub admin    | Onboarding review (list, detail, decide APPROVE / REJECT)                              |
| [admin-onboarding-questions.md](./admin-onboarding-questions.md) | Internal Hub admin    | CRUD + active/inactive toggle + soft delete + bulk reorder on the global question bank |
...
3. For 3 months, **login**, **register**, and **profile-submit** all return `403 CONSULTANT_ONBOARDING_BLOCKED` with `details.blocked_until`. See [auth/consultant.md](../auth/consultant.md#onboarding-rejection-block-read-first).
4. After the block expires the consultant may re-register / re-onboard.

## Companion docs

- [Auth — Consultant](../auth/consultant.md) — login behaviour around the rejection block.
- [Notifications — Consultant Event Catalog](../notifications/notifications-consultant-events.md) — onboarding_approved notification.
- [Files — Upload + Purpose](../files/) — shared `/files` upload endpoint.
