...
> **Source:**
> [src/modules/admin-consultant-onboarding/controllers/admin-consultant-onboarding.controller.ts](../../../../src/modules/admin-consultant-onboarding/controllers/admin-consultant-onboarding.controller.ts)
...
| 422  | (validation)                           | DTO failed validation (UUIDs, enum values, length limits).           |
...
- **Query params:** [`ListOnboardingsDto`](../../../../src/modules/admin-consultant-onboarding/dto/requests/list-onboardings.dto.ts)

  | Field     | Type   | Required | Notes                                                                                                 |
  | --------- | ------ | -------- | ----------------------------------------------------------------------------------------------------- |
  | `status`  | enum   | no       | When omitted, defaults to `INTERVIEW_SUBMITTED` (pending-review queue). Pass empty string to disable. |
  | `user_id` | uuid   | no       | Filter to a single consultant.                                                                        |
  | `page`    | int ≥1 | no       | Default `1`.                                                                                          |
  | `take`    | int    | no       | Default `20`, min `1`, max `100`.                                                                     |

  `status` valid values: `PENDING_BASIC_INFO`, `IN_INTERVIEW`, `INTERVIEW_SUBMITTED`, `APPROVED`, `REJECTED`.
...
- **Path params:** `id` (UUID v4) — the onboarding id.
- **Response 200:** [`OnboardingDetailResponseDto`](../../../../src/modules/admin-consultant-onboarding/dto/responses/onboarding-detail-response.dto.ts)
...
- **Body:** [`OnboardingDecisionDto`](../../../../src/modules/admin-consultant-onboarding/dto/requests/onboarding-decision.dto.ts)
...
- **Response 200:** the refreshed [`OnboardingDetailResponseDto`](../../../../src/modules/admin-consultant-onboarding/dto/responses/onboarding-detail-response.dto.ts). On reject the `blocked_until` field is now set to `now + 3 months`.
...
- **Side effects:**
  ...
  | REJECTED | `onboarding.status = REJECTED`, `reviewed_by/reviewed_at`, `rejection_note`, `blocked_until = now + 3 months`. | Email: `sendApplicationRejectedEmail` with `reason` + `blocked_until`. Auth flow then refuses login / register / profile-submit for 3 months — see [consultant auth](../../consultant/auth/auth-api-specs.md#onboarding-rejection-block-read-first). |
...
- **Errors:**
  ...
  | 422 | (validation) | `decision` is not one of `APPROVED \| REJECTED`, or `rejection_note` exceeds 2000 chars. |
