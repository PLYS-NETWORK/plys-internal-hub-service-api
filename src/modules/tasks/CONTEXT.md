...
> **Business surface moved.** The business-side task endpoints (create draft, assign, status transitions, comments) now live in `BusinessProjectsModule` under `/projects/business/:id/board/...`. The flows below still describe the underlying behaviour because the new controllers delegate into the same shared services in this module.
...
Triggered when the business moves a task out of draft via the project-board surface in `BusinessProjectsModule`.
...
Triggered when the business approves a task through the project-board surface in `BusinessProjectsModule`.
