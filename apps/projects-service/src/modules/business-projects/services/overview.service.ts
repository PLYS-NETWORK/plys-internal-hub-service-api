...
export class BusinessProjectOverviewService implements IBusinessProjectOverviewService {
  ...
  public async getOverview(projectId: string): Promise<OverviewResponseDto> {
    ...
    const [
      ...
      this.profilesReader.findConsultantSkillsByIds(consultantIds),
    ...
    for (const row of consultantSkills) {
      const list = skillsByConsultant.get(row.consultantId) ?? [];
      list.push({
        consultant_id: row.consultantId,
        skill_id: row.skillId,
        skill_name: row.skillName,
        proficiency_level: row.proficiencyLevel as ProficiencyLevel | null,
        rating: row.rating,
      });
      skillsByConsultant.set(row.consultantId, list);
    }
