...
| -------------------------------- | --------------------- | ---------------------------------------------------------------------- |
| [consultant.md](./consultant.md) | Consultant (Lona app) | Register, view current exam, eligibility, submit answers, submit exam. |
| [admin.md](./admin.md)           | Internal Hub admin    | Read-only list + detail of every skill exam across consultants.        |
...
Every terminal skill-exam transition (PASSED / FAILED / EXPIRED / COPYLEAKS_FAILED) and every CopyLeaks-ban fans out an in-app notification to every active admin. See [Notification Type Catalog](../notifications/) for the `admin_skill_exam_result` and `admin_consultant_banned` payloads.

## Companion docs

- [Auth — Consultant](../auth/consultant.md) — banned-account login behaviour.
- [Notifications](../notifications/) — consultant + admin notification catalogs.
