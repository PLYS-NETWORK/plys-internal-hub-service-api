...
> **Source:** [apps/consultant-service/src/modules/consultant-projects/controllers/consultant-joined-projects.controller.ts](../../../../apps/consultant-service/src/modules/consultant-projects/controllers/consultant-joined-projects.controller.ts)
...
Unlike the explore endpoints — which surface projects in `PUBLISHED` / `IN_PROGRESS` to any consultant — these endpoints require the caller to be an **`ACTIVE` member** of the project. Implementation lives in [`ConsultantAccessService.resolveJoinedProject`](../../../../apps/consultant-service/src/modules/consultant-projects/services/consultant-access.service.ts):
...
All three routes use the shared [`THROTTLE_DISCOVERY`](../../../../apps/consultant-service/src/common/constants/throttle.constants.ts) tier — these are reads, surfaced on every workspace navigation.
...
| 404  | `PROJECT_NOT_FOUND`            | Detail endpoint only. Caller has no `ACTIVE` `project_members` row for the project, OR the project is missing/soft-deleted. Thrown by [ConsultantAccessService.resolveJoinedProject](../../../../apps/consultant-service/src/modules/consultant-projects/services/consultant-access.service.ts). |
...
- **Response 200:** [`IConsultantWorkspaceListItemResponse[]`](../../../../apps/consultant-service/src/modules/consultant-projects/dto/responses/interfaces/consultant-workspace-list-item.response.interface.ts) (plain array — no `PageDto` envelope around it; the `data` field of the standard response holds the array directly)
...
- **Throttle:** 60 req / 60s
- **Cache:** 60s
- **Query params:** [`ListConsultantJoinedProjectsDto`](../../../../apps/consultant-service/src/modules/consultant-projects/dto/requests/list-consultant-joined-projects.dto.ts) extends `PageOptionsDto`. Same shape as workspaces (`page`, `limit`, `keyword`).
...
- **Behaviour:**
  ...
  - Two aggregate queries run in parallel for the result set:
    1. [`TaskRepository.countCompletionByProjectIdsGroupedByProject`](../../../../packages/unit-of-work/repositories/tasks/task.repository.ts) — `(total_tasks, completed_tasks)` per project. `completion_pct` = `round(completed / total * 100)` (or `0` when there are no tasks).
    2. [`TaskRepository.countCompletedByAssigneeAndProjectIds`](../../../../packages/unit-of-work/repositories/tasks/task.repository.ts) — count of DONE tasks per project assigned to the caller.

- **Response 200:** `PageDto<`[`IConsultantJoinedProjectListItemResponse`](../../../../apps/consultant-service/src/modules/consultant-projects/dto/responses/interfaces/consultant-joined-project-list-item.response.interface.ts)`>`
...
- **Behaviour:**
  ...
  - Five queries run in parallel:
    1. [`BusinessProfileRepository.findOne`](../../../../packages/unit-of-work/repositories/profiles/business-profile.repository.ts) — for `company_name`.
    2. [`ProjectRequiredSkillRepository.findWithSkillByProjectId`](../../../../packages/unit-of-work/repositories/projects/project-required-skill.repository.ts) — required skills with their `Skill` rows, translated into the request locale.
    3. [`TaskRepository.countByProjectIdsGroupedByStatus`](../../../../packages/unit-of-work/repositories/tasks/task.repository.ts) — project-wide status histogram. `total_tasks` = sum of every non-DRAFT bucket. `completed_tasks_overall` = `done` bucket.
    4. [`TaskRepository.countByAssigneeAndProjectGroupedByStatus`](../../../../packages/unit-of-work/repositories/tasks/task.repository.ts) — caller's own status histogram. `completed_tasks_by_me` = `done` bucket. `in_progress_by_me` = `in_progress` bucket.
    5. [`ProjectMemberRepository.countActiveTotalByProjectIds`](../../../../packages/unit-of-work/repositories/projects/project-member.repository.ts) — `total_members` (ACTIVE only).
  - `completion_pct` = `round(completed_tasks_overall / total_tasks * 100)`, or `0` when `total_tasks = 0`.

- **Response 200:** [`IConsultantJoinedProjectDetailResponse`](../../../../apps/consultant-service/src/modules/consultant-projects/dto/responses/interfaces/consultant-joined-project-detail.response.interface.ts)
