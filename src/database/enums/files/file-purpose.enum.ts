 ...
 *
 * `CONSULTANT_CV` and `AVATAR` are set by the client at upload time
 * (the only purposes accepted on POST /files): when present, the storage
 * key is prefixed with `consultant-CVs/<env>/` or `avatars/<env>/`
 * instead of the default sharded path.
 */
