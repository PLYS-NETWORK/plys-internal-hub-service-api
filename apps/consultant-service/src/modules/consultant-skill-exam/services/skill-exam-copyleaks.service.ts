...
export class SkillExamCopyleaksService implements ISkillExamCopyleaksService {
  ...
  public async run(examId: string): Promise<void> {
    ...
    this.notificationsClient.emit(NOTIFICATION_EVENTS.CONSULTANT_SKILL_EXAM_FAILED, failedPayload);
    ...
    if (result2.strikeCount >= BAN_STRIKE_THRESHOLD && result2.bannedAt) {
      ...
      this.notificationsClient.emit(NOTIFICATION_EVENTS.CONSULTANT_ACCOUNT_BANNED, bannedPayload);
    }
