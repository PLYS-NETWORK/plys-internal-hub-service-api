...
- **Response 200:** `PageDto<`[`ITransactionResponse`](../../../src/modules/payments/dto/responses/interfaces/transaction.response.interface.ts)`>`
  ...
  **Item fields:**
  ...
  | `created_at`        | `ISO 8601`                | no       | Timestamp formatted in the caller's resolved timezone (`business_profile.timezone` → `x-timezone` header → `UTC`). Offset included, e.g. `2026-04-20T19:00:00.000+07:00`. |
