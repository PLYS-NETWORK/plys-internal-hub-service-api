...
- **Behaviour:**
  1. Creates a `TOP_UP` `BusinessTransaction` with `status = PENDING` (transaction number generated atomically).
  2. Calls Polar `createCheckoutSession`; saves the `processorInvoiceId` on the transaction row.
  3. Returns the checkout redirect URL for the frontend to navigate the user to.
  4. On checkout success, the Polar webhook handler transitions the transaction to `COMPLETED` and credits `accountBalance` — **this response does not confirm payment**.
...
- **Behaviour:**
  ...
  3. Calls Polar `createCheckoutSession`; saves processor IDs on both the invoice and the transaction row.
  4. The webhook handler marks the invoice `PAID` and the transaction `COMPLETED` asynchronously after the user completes payment.
