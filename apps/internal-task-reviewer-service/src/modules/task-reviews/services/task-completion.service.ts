...
import { ITaskStatusChangedEvent, NOTIFICATION_EVENTS } from '@plys/libraries/common-nest/events';
...
import { Task } from '@plys/libraries/database/entities';
import {
...
export class TaskCompletionService implements ITaskCompletionService {
  ...
  public async markDone(taskId: string): Promise<void> {
    ...
    const result = await this.uow.withTransaction(async (tx) => {
      ...
      const consultantProfile = await this.profilesLedger.findConsultantInTransaction(
        saved.assignedTo!,
        tx,
      );
      ...
      await this.profilesLedger.creditConsultant(saved.assignedTo!, payoutAmount, tx);
      ...
      const businessProfile = project
        ? await this.profilesLedger.findBusinessInTransaction(project.businessId, tx)
        : null;
  ...
  public async markRevisionRequested(taskId: string, feedbackSummary: string): Promise<void> {
    ...
    const consultantProfile = result.task.assignedTo
      ? await this.profilesReader.findConsultantById(result.task.assignedTo)
      : null;
    const project = await this.uow.projects.findOne({ where: { id: result.task.projectId } });
    const businessProfile = project
      ? await this.profilesReader.findBusinessById(project.businessId)
      : null;
  ...
  private emitStatusChangedEvent(params: {
    ...
    this.notificationsClient.emit(NOTIFICATION_EVENTS.TASK_STATUS_CHANGED, payload);
  }
